...
Pass this file to [`train --config PATH`](../reports/reproduction.md#2-rebuild-data-and-the-selected-recipes). The command saves its original bytes and effective settings alongside the constructed encoder and selected checkpoint. Configuration changes require a distinct run directory.

Optional [MLflow tracking](../reports/reproduction.md#2-rebuild-data-and-the-selected-recipes) is configured through `--mlflow-dir` and `--experiment-name`, separately from this model configuration. Its storage paths and run identity are recorded in the local manifest.
...
The selected neural recipe is [mean-pool-mlp-regularized-v1.toml](experiments/mean-pool-mlp-regularized-v1.toml): dropout `0.3`, weight decay `0.1`, 128-dimensional embeddings, and hidden dimension 64. It uses `artifacts/preprocessing/vocabulary-v1/` and `--max-length 128`. The [selection record](../reports/mlp-selection-v1/selection.json) pins its full identity and seed-17 checkpoint; the configuration file remains the original registered recipe.
